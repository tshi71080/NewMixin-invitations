...
public abstract class EaseChatRow extends LinearLayout {
    ...
    private void initView() {
        ...
        userTypeView = (TextView) findViewById(R.id.tv_user_type);
            ...
            if (message.direct() == Direct.SEND) {
                ...
                if (message.getChatType() == EMMessage.ChatType.GroupChat) {
                    ...
                    if (isCurrentOwner(group)) {
                        ...
                        userTypeView.setText("群主");
                    } else if (isCurrentAdmin(group)) {
                        userTypeView.setVisibility(View.VISIBLE);
                        userTypeView.setText("管理员");
                    } else {
                ...
                try {
                    //String markName = "";
                    int user_type = message.getIntAttribute("user_type");
                    String nickname = message.getStringAttribute("real_name");

                    switch (user_type) {
                        case 0:
                            userTypeView.setVisibility(View.VISIBLE);
                            userTypeView.setText(nickname);
                            break;
                        case 1:
                            userTypeView.setVisibility(View.VISIBLE);
                            userTypeView.setText(nickname + "(群主)");
                            break;
                        case 2:
                            userTypeView.setVisibility(View.VISIBLE);
                            userTypeView.setText(nickname + "(管理员)");
                            break;
                    }
                    Glide.with(context).load(message.getStringAttribute("photo_path")).asBitmap().into(userAvatarView);
                    /*if(EaseUserUtils.getRemarkBeanList()!=null&&EaseUserUtils.getRemarkBeanList().size()>0){
                        ...
                        for(MyRemarkFriendBean remarkFriendBean:EaseUserUtils.getRemarkBeanList()){
                            if(remarkFriendBean.getIm_name().equals(message.getFrom())){
                                markName = remarkFriendBean.getRemark();
                            }
                        }
                    }

                    if(!TextUtils.isEmpty(markName)){
                        usernickView.setText(markName);
                    }else{*/
                    usernickView.setText(message.getStringAttribute("real_name"));
                    //}
