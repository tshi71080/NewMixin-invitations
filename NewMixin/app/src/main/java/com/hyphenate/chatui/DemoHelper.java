...
import com.hyphenate.chatui.domain.InviteMessage.InviteMessageStatus;
...
import com.hyphenate.chatui.receiver.HeadsetReceiver;
...
//                break;
 ...
 /*   private EaseUser getUserInfo(String username) {
    ...
    private EaseUser getUserInfo(String username) {
        ...
        //设置自己的头像
        UserBean loginUser = AccountUtils.getUser(appContext);

        if (username.equals(EMClient.getInstance().getCurrentUser())) {
            user = new EaseUser(username);
            user.setAvatar(loginUser.getPhoto_path());
            user.setNickname(loginUser.getReal_name());
            return user;
        } else {
            //设置别人的头像
    ...
    //请求服务器获取用户昵称以及头像信息
    private void getUserInfoFromServer(final String username) {
        HttpParams httpParams = new HttpParams();
        httpParams.put("token", AccountUtils.getUserToken(appContext));
        httpParams.put("im_name", username);
        OkGoRequest.post(UrlUtils.getUserInfo, appContext,
                httpParams, new JsonCallback<LazyResponse<UserInfo>>(appContext, false) {
                    @Override
                    public void onSuccess(LazyResponse<UserInfo> bean, Call call, Response response) {
                        super.onSuccess(bean, call, response);
                        if (bean.getStatus() == 1) {
                            String nickname = bean.getData().getReal_name();
                            String avatar = bean.getData().getPhoto_path();

                            EaseUser user = new EaseUser(username);
                            //遍历循环查找备注名字
                            String remarkname = "";
                            if(EaseUserUtils.getRemarkBeanList()!=null&&EaseUserUtils.getRemarkBeanList().size()>0){
                                for(MyRemarkFriendBean remarkFriendBean:EaseUserUtils.getRemarkBeanList()){
                                    if(username.equals(remarkFriendBean.getIm_name())){
                                        remarkname = remarkFriendBean.getRemark();
                                    }
                                }
                            }
                            if(!TextUtils.isEmpty(remarkname)){
                                user.setNickname(remarkname);
                            }else{
        ...
        messageListener = new EMMessageListener() {
            ...
            public void onCmdMessageReceived(List<EMMessage> messages) {
                for (EMMessage message : messages) {
                    ...
                    //收到禁言透传信息
                    if(action.equals("com.mixin.mute")){
                        EventBus.getDefault().post(Constants.MUTE_INFO.MUTE);
                        //EventBus.getDefault().post();
                    }else if(action.equals("com.mixin.unmute")){
                        EventBus.getDefault().post(Constants.MUTE_INFO.UN_MUTE);
                    }
                    //收到添加好友的请求
                    if(action.equals("applyFriend")){
