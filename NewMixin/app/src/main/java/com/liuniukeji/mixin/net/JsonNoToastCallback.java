 ...
 * ================================================
 */
public abstract class JsonNoToastCallback<T> extends AbsCallback<T> {
     ...
     * @param canShow
     */
    public JsonNoToastCallback(Context context, boolean canShow) {
     ...
     * @param errorView
     */
    public JsonNoToastCallback(Context context, boolean canShow, View errorView) {
        this.context = context;
        this.canShow = canShow;
     ...
     * @param errorView
     */
    public JsonNoToastCallback(Context context, View loadingView, View errorView) {
        this.context = context;
        this.loadingView = loadingView;
        this.errorView = errorView;
    }

    public JsonNoToastCallback(Context context, PtrLayout ptrView, View errorView) {
        this.context = context;
        this.ptrLayout = ptrView;
        this.errorView = errorView;
    }
...
//        request.getUrlParam("json");
//        Log.e(params.toString());
        String s = request.getUrlParam("code");
        request.params("code", s);
        mResponse = "";
    ...
    public void onError(Call call, Response response, Exception e) {
        ...
        try {
            String info = JSON.parseObject(mResponse).get("info").toString();
//            if (info.contains("用户不合法")) {
...
//            SimpleResponse simpleResponse = JSON.parseObject(response.body().string(), SimpleResponse.class);
            ...
            SimpleResponse simpleResponse = Convert.fromJson(mResponse, SimpleResponse.class);
            response.close();
...
//                if (  TextUtils.isEmpty(simpleResponse.toLazyResponse().getInfo())||simpleResponse.toLazyResponse().getInfo().equals("暂无数据")){
//
//                }else {
...
//                }
        ...
        } else if (rawType == LazyResponse.class) {
            //有数据类型，表示有data
//            LazyResponse lzyResponse = (LazyResponse)JSON.parseObject(response.body().string(), LazyResponse.class);
            mResponse = response.body().string();
            LazyResponse lzyResponse = Convert.fromJson(mResponse, type);
            response.close();
...
//                ToastUtils.showShortToastSafe(lzyResponse.getInfo());
                throw new IllegalStateException(lzyResponse.getInfo());
            } else if (code == 104) {
