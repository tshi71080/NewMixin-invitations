...
import vn.tungdx.mediapicker.utils.PreferencesUtils;
...
public class MediaPickerFragment extends BaseFragment implements
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position,
                            long id) {
        mMediaAdapter.removeMediaSelected();
        ...
        if (object instanceof Cursor) {
            ...
            /*String maxmax = "";
            if (PreferencesUtils.getString(getContext(), "video_from") != null && !PreferencesUtils.getString(getContext(), "video_from").isEmpty()) {
                maxmax = PreferencesUtils.getString(getContext(), "video_from");
            } else if (PreferencesUtils.getString(getContext(), "video_renzheng") != null && !PreferencesUtils.getString(getContext(), "video_renzheng").isEmpty()) {
                maxmax = PreferencesUtils.getString(getContext(), "video_renzheng");
            }
            int max_toast = Integer.parseInt(maxmax);
            double max = Double.valueOf(maxmax) + 0.1;
            if (mediaItem.getTimeLength(getActivity()) > max * 1000) {
                // Toast.makeText(getActivity(), R.string.shiPinShiJianBiXuXiaoYuXXMiao, Toast.LENGTH_SHORT).show();
                Toast.makeText(getActivity(), String.format(getResources().getString(R.string.shiPinShiJianBiXuXiaoYuXXMiao
                ), max_toast), Toast.LENGTH_SHORT).show();
                mMediaSelectedListener.onHasNoSelected();
                return;
            }*/

            mMediaSelectedListener.onHasNoSelected();
            mMediaAdapter.updateMediaSelected(mediaItem, pickerImageView);  //如果是选中项目则更改为位选中
    ...
    private void switchToError() {
        ...
        mNoItemView.setTextColor(Color.parseColor("#000000"));
    ...
    public class MyBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            video_from = intent.getStringExtra("video");
            PreferencesUtils.putString(context, "video_from", video_from);
        }
    }
    ...
    * 认证的视频
    * */
    String video_renzheng = "";

    public class RenBroadcastReceiver extends BroadcastReceiver {

        @Override
        public void onReceive(Context context, Intent intent) {
            video_renzheng = intent.getStringExtra("video_renzheng");
            PreferencesUtils.putString(context, "video_renzheng", video_renzheng);
        }
    }
}