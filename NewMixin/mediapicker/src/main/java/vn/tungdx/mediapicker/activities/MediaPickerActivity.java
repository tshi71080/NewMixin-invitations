...
public class MediaPickerActivity extends AppCompatActivity implements
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getSupportActionBar() != null) {
            ...
            getSupportActionBar().setDisplayHomeAsUpEnabled(true);
            getSupportActionBar().setHomeAsUpIndicator(R.drawable.back_arrow1);
        }
    }
    ...
    private void takeVideo() {
        ...
        mSharedPreferences = getSharedPreferences("video_data", Context.MODE_PRIVATE);
        mEditor = mSharedPreferences.edit();
        final Intent takeVideoIntent = new Intent(MediaStore.ACTION_VIDEO_CAPTURE);
        takeVideoIntent.putExtra(MediaStore.EXTRA_VIDEO_QUALITY, 0);
//        takeVideoIntent.putExtra("camerasensortype", 2); // 调用前置摄像头
        if (takeVideoIntent.resolveActivity(getPackageManager()) != null) {
//            int max = mMediaOptions.getMaxVideoDuration();
            String maxmax = "";
            if (PreferencesUtils.getString(this, "video_from") != null && !PreferencesUtils.getString(this, "video_from").isEmpty()) {
                maxmax = PreferencesUtils.getString(this, "video_from") + "000";
            } else if (PreferencesUtils.getString(this, "video_renzheng") != null && !PreferencesUtils.getString(this, "video_renzheng").isEmpty()) {
                maxmax = PreferencesUtils.getString(this, "video_renzheng") + "000";
            }
            int max = Integer.parseInt(maxmax);
            if (max != Integer.MAX_VALUE) {
                // /=1000 because it's must in seconds.
                max /= 1000;
                takeVideoIntent.putExtra(MediaStore.EXTRA_DURATION_LIMIT, max);    //llt 最大时长
